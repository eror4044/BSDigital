...
export function useOrderBook() {
  ...
  async function fetchQuote() {
    try {
      const resp = await fetch(`${API_URL}/api/quotes?amount=${amountBtc.value}&type=buy`, {
        credentials: "omit"
      });
  ...
  async function connect() {
    const connection = new signalR.HubConnectionBuilder()
      .withUrl(`${API_URL}/hubs/orderbook`, { transport: signalR.HttpTransportType.WebSockets })
    ...
    connection.on("orderbook:update", (data: { bids: [string, string][], asks: [string, string][] }) => {
      ...
      const parsedBids = data.bids
        ...
        .map(([p, v]) => [parseFloat(p), parseFloat(v)] as [number, number])
        .sort((a, b) => b[0] - a[0]);
